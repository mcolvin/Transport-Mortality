...
pdat<- lapply(1:nrow(prds),function(x)
    {
    if(prds$standardize[x]==1){xx<-runif(n,-2,2)}
    if(prds$standardize[x]==0 & prds$pred[x]!=1){xx<-sample(unique(dat$trip_no),
        n,replace=TRUE)}
    if(prds$standardize[x]==0 & prds$pred[x]==1){xx<-rep(1,n)}  
    return(xx)
    })
...
## 
y<-matrix(0,nrow=n,ncol=nrow(prds))
pdat<- lapply(1:ncol(yy),function(x)
    {
    mm<- paste("~",paste(names(yy[x]),collapse="+"))
    mm<- model.matrix(as.formula(mm),yy)	
    y_hat[,x]<- mm%*%fixef(out_foster[[mod]])	
