...
* see line 78 need to link predictions and numbers ouplanted
and then discretize
-->
...
yy$ntrap<- sample(1:max(dat_unstd[dat_unstd$location=='Foster Dam',]$trap_total),
    n, replace=TRUE)
```
...
y_hat<-matrix(0,nrow=n,ncol=nrow(confModSet))

strt<-seq(1,n+2,by=10000)
end<-seq(1,n+2,by=10000)-1
ppp<- cbind(strt[-length(strt)],end[-1])
## SIMUALATE OUTCOMES: SURVIVAL PROBABILITY
for(j in 1:nrow(ppp))
    {
    for(i in 1:length(confModSet$model_indx))
        {
        nn<- length(ppp[j,1]:ppp[j,2])
        mm<- paste("~",paste(confModSet$pred[i],collapse="+"))
        mm<- model.matrix(as.formula(mm),yy[ppp[j,1]:ppp[j,2],])   
        
        ## INDEX FOR MODEL OUTPUT
        x<- confModSet$model_indx[i]
        pvar1 <- diag(mm %*% tcrossprod(as.matrix(vcov(out_foster[[x]])),mm))
        tvar1 <- pvar1+VarCorr(out_foster[[x]])$samp[1] 
        
        ## y_hat pr survival in order of weights
        y_hat[ppp[j,1]:ppp[j,2],i]<-plogis(rnorm(nn,mm%*%fixef(out_foster[[x]]),tvar1))
        }
    }
...
for(i in 1:ncol(y_hat))
    {
    n_hat[,i]<-rbinom(n,yy$ntrap,y_hat[,i])
    }
## SURVIVORS ACCOUNTING FOR MODEL UNCERTAINTY
yy$n_hat_weighted<- n_hat %*% confModSet$w
