...
y_hat<-matrix(NA,nrow=n,ncol=nrow(confModSet))
ys<- y_hat[-1,]
...
for(j in 1:1)
    ...
    for(i in 1:length(confModSet$model_indx))
        ...
        mm<- model.matrix(as.formula(mm),yy)   
        
        ## INDEX FOR MODEL OUTPUT
        x<- confModSet$model_indx[i]
        pvar1 <- diag(mm %*% 
            tcrossprod(as.matrix(vcov(out_foster[[x]])),mm))
        tvar1 <- pvar1+VarCorr(out_foster[[x]])$samp[1] 
        
        ## y_hat pr survival in order of weights
        y_hat[,i]<-plogis(rnorm(n,mm%*%fixef(out_foster[[x]]),tvar1))
        }
    ys<- rbind(ys,y_hat)
    }
...
R<- matrix(NA,nrow=ndec, ncol=nstates)  
